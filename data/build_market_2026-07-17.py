#!/usr/bin/env python3
"""Build market_2026-07-17.json from DB + tech data."""
...
db = MarketDB()
tech = json.load(open(os.path.join(BASE, "market_tech_2026-07-17.json")))
...
credibility = [
    ...
    {"item":"板块行情(QQ+westock)","status":"✅","note":f"{len(sector_chg)}条(5 QQ实时+4 cs westock+北证50)"},
    {"item":f"情绪因子(FG={fg})","status":"✅","note":"sentiment表"},
    {"item":"市场广度","status":"⚠","note":"无数据(按中性50计)"},
    {"item":"北向资金","status":"⚠","note":"净流出18.5亿"},
    {"item":"主力资金流向","status":"⚠","note":"暂无数据"},
    {"item":"板块OHLC(westock+QQ)","status":"✅","note":"cs板块westock今日实时+QQ今日实时"},
]

# ── Final JSON ──
out = {
    "date": TODAY,
    "updated_at": f"{TODAY} 收盘",
    "data_tier": "T2",
    "core": core,
    ...
    "risks": [],
    "disclaimer": f"⚠ 本分析仅供参考,预测为模型研判,历史不代表未来。数据含T2估算与AI模型推断,不构成投资建议。涨红跌绿按A股惯例。板块cs数据westock今日实时。{TODAY}。"
}
